...
df_filtered = df[(df['efficiency_hll'] > 0) & (~df['efficiency_hll'].isna())].copy()
...
# Stampa i valori di efficiency_hll divisi per categoria e num_threads
...
for category in sorted(df_filtered['nz_category'].unique()):
    ...
    for num_threads in sorted(category_data['num_threads'].unique()):
        avg_efficiency = category_data[category_data['num_threads'] == num_threads]['efficiency_hll'].mean()
        print(f"Threads: {num_threads}, Efficienza media: {avg_efficiency:.4f}")

# Genera un grafico per visualizzare l'efficienza
groups = df_filtered.groupby(['nz_category', 'num_threads'])['efficiency_hll'].mean().reset_index()
...
for category in sorted(df_filtered['nz_category'].unique()):
    ...
    plt.plot(subset['num_threads'], subset['efficiency_hll'],
...
try:
    plt.savefig("efficiency_by_matrix_size_hll.png", dpi=300, bbox_inches='tight')
